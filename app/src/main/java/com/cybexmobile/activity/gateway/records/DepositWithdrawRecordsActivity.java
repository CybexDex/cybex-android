...
public class DepositWithdrawRecordsActivity extends BaseActivity implements OnRefreshListener, OnLoadMoreListener {
    ...
    private DepositWithdrawRecordOldAdapter mDepositWithdrawRecordAdapterOld;
    private List<GatewayNewDepositWithdrawRecordItem> mRecordsItems = new ArrayList<>();
    private List<GatewayDepositWithdrawRecordsItem> mRecordsItemsOld = new ArrayList<>();
    ...
    public void onLoadMore(@NonNull RefreshLayout refreshLayout) {
        if ((SettingConfig.getInstance().isGateway2() ? mDepositWithdrawRecordAdapter.getItemCount() : mDepositWithdrawRecordAdapterOld.getItemCount()) == mTotalItemAmount) {
    ...
    private void loadListNewGateWay() {
...
//                        Log.v("gatewayRequestBody", request);
                        return RetrofitFactory.getInstance()
                                ...
                                .getDepositWithdrawRecords(
                                        ...
                                        "bearer " + mSignature,
                                        mAccountName,
                                        mIsRefresh && mRecordsItemsOld.size() > LOAD_COUNT ? mRecordsItemsOld.size() : LOAD_COUNT,
                                        mIsRefresh ? 0 : mRecordsItemsOld.size(),
                                        mAssetObject.symbol,
                                        mFundType,
                                        false,
                                        false
                ...
                .subscribe(new Consumer<List<GatewayDepositWithdrawRecordsItem>>() {
                    ...
                    public void accept(List<GatewayDepositWithdrawRecordsItem> gatewayDepositWithdrawRecordsItems) {
                        ...
                        mRefreshLayout.finishLoadMore();
                        if (mIsRefresh) {
                            mRecordsItemsOld.clear();
                            mRecordsItemsOld.addAll(gatewayDepositWithdrawRecordsItems);
                        } else {
                            mRecordsItemsOld.addAll(gatewayDepositWithdrawRecordsItems);
                        }
                        if (mDepositWithdrawRecordAdapterOld == null) {
                            mDepositWithdrawRecordAdapterOld = new DepositWithdrawRecordOldAdapter(DepositWithdrawRecordsActivity.this, mRecordsItemsOld);
                            mDepositRecordsRecyclerView.setAdapter(mDepositWithdrawRecordAdapterOld);
                        } else {
                            mDepositWithdrawRecordAdapterOld.notifyDataSetChanged();
                        }
                    }
                }, new Consumer<Throwable>() {
                    @Override
